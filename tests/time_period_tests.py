...
class TimePeriodTests(unittest.TestCase):
    ...
    def test_custom_periods(self):
        ...
        # Sunday
            ...
            [datetime(2016, 4, 6, 10, 20, 10),
                 [
                     ...
                     ['last 2 wednesdays', datetime(2016, 3, 23)],
                     ...
                     ['next 2 wednesdays', datetime(2016, 4, 20)],
        ...
        # random generator
        def rnd_gen(p):
            refDate = datetime.now() + relativedelta(days=int(random.random() * 10))
            refDate = refDate.replace(microsecond=0)
            delta = int(random.random() * 7)
            direcction = bool(random.getrandbits(1))
            unitType = ["years", "months", "days", "weeks"]
            unit = unitType[int(random.random()*len(unitType))]
            args = {}
            offset = delta
            if offset == 0:
                offset = 1
            if not direcction:
                offset *= -1
            args[unit] = offset
            expected = refDate + relativedelta(**args)
            exp = ""
            if unit in ["years", "months", "weeks"]:
                this_day = bool(random.getrandbits(1))
                if this_day:
                    exp += "this day "
                else:
                    if unit == "months":
                        expected = datetime(expected.year, expected.month, 1,
                                            expected.hour, expected.minute, expected.second)
                    elif unit == "weeks":
                        expected += relativedelta(days=-1 * expected.weekday())
                    else:
                        expected = datetime(expected.year, 1, 1, expected.hour, expected.minute, expected.second)
            if direcction:
                exp += "next "
            else:
                exp += "last "
            # compose expression
            zero_def = bool(random.getrandbits(1))
            if delta == 0 and zero_def:
                exp += unit[:len(unit)-1]
            else:
                exp += str(delta) + " " + unit
            # so far ?
            so_far = bool(random.getrandbits(1))
            if not so_far:
                expected = datetime(expected.year, expected.month, expected.day)
            else:
                exp += " so far"

            test = [refDate, [[exp, expected]]]
            p.append(test)
        ...
        for now, dataset in periods:
            for p, d in dataset:
                ...
                self.assertEqual(res, d, "For date: {}. Test: '{}' returned: {} expected: {}".format(now, p, res, d))
